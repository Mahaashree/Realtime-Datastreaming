...
class AdaptiveThreadAllocator:
    ...
    def calculate_allocation(self, offline_queue_size: int) -> tuple:
        ...
        - 0 messages: 8 live, 0 offline (optimal - no backlog)
        - 1-1,000: 7 live, 1 offline (minimal backlog)
        - 1,001-5,000: 6 live, 2 offline (small backlog)
        - 5,001-20,000: 5 live, 3 offline (medium backlog)
        - 20,000+: 4 live, 4 offline (large backlog - 50% minimum guarantee)
        """
        if offline_queue_size == 0:
            return (8, 0)  # All threads for live
        elif offline_queue_size <= OFFLINE_THRESHOLD_MINIMAL:
            return (7, 1)  # Minimal backlog
        elif offline_queue_size <= OFFLINE_THRESHOLD_SMALL:
            return (6, 2)  # Small backlog
        elif offline_queue_size <= OFFLINE_THRESHOLD_MEDIUM:
            return (5, 3)  # Medium backlog
        else:
            return (4, 4)  # Large backlog (50/50 split, live guaranteed 50%)
...
class MQTTCollectorWithMonitoring:
    ...
    def _init_influxdb(self):
        """Initialize InfluxDB connection with async writer."""
        try:
            self.influx_client = InfluxDBClient(
                ...
                connection_pool_maxsize=5,     # Reduced connection pool for stability
        ...
        # Create synchronous write API for async writer with stable settings
        self.write_api = self.influx_client.write_api(
            write_options=WriteOptions(
                batch_size=INFLUXDB_BATCH_SIZE,
                flush_interval=INFLUXDB_FLUSH_INTERVAL,
                jitter_interval=200,           # Increased jitter for stability
                retry_interval=3000,           # Longer retry interval for stability
                max_retries=3,                 # Conservative retry count
                max_retry_delay=10000,         # Conservative max retry delay
                exponential_base=2             # Standard exponential backoff
            )
        )
        
        # Initialize async writer - ULTRA LOW LATENCY
        self.async_writer = AsyncInfluxWriter(
            write_api=self.write_api,
            bucket=INFLUXDB_BUCKET,
            max_workers=8,  # Increase to 8 for maximum parallelism
            batch_size=10,  # Ultra-small batches for instant writes
            flush_interval=0.01,  # 10ms flush for ultra-low latency
            max_queue_size=2000,  # Balanced queue size for optimal throughput
            success_callback=self._write_success_callback,
            error_callback=self._write_error_callback
        )
        
        self.logger.info("AsyncInfluxWriter initialized", 
                        workers=8,
                        batch_size=10, 
                        flush_interval_s=0.01)
    
    def _write_success_callback(self, batch_size: int, duration: float, batch_metadata: list):
        """Called when async write succeeds."""
                ...
                for publish_ts in metadata['publish_timestamps']:
                    if publish_ts:
                        ...
                        if 0 < full_latency < 60:
                            ...
                            self.metrics.record_message_processed(full_latency, pipeline=pipeline)
        ...
        try:
            while not self.shutdown_event.is_set():
                ...
                try:
                    ...
                    for _ in range(OFFLINE_BATCH_SIZE - 1):
                        try:
    ...
    def _process_batch(self, batch, pipeline='live', queue=None):
        ...
        if points:
            ...
            # Submit to async writer - returns immediately
            success = self.async_writer.write_async(points, metadata)
    ...
    def _shutdown(self):
        ...
        try:
            # Shutdown async writer first (flushes remaining batches)
            self.async_writer.shutdown(timeout=30)
